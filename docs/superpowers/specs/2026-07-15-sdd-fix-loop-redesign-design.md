...
> **Ultra fork status:** Imported as upstream historical evidence. The fork
> adopts scoped re-review mechanics but rejects the fixed five-round breaker,
> forced fresh Implementers, fixed model escalation, and mandatory per-task
> review described below. Current behavior is governed by `docs/ultra-fork.md`
> and the 2026-07-23 semantic integration design.
